...
# sudo python3 ./test_matrix.py --led-rows=64 --led-cols=64 --led-chain=4 --led-multiplexing=1 --led-brightness=20 --led-slowdown-gpio=4
...
class SimpleSquare(SampleBase):
    ...
    def run_(self):
        ...
        print("dims:", self.matrix.width, "-", offset_canvas.width, offset_canvas.height)
        while True:
    ...
    def run(self):
        ...
        m = infra_c.IntMatrix(128, 128)

        # pattern = "rand"
        pattern = "square"

        if pattern == "rand":
            for yi in range(0, 128):
                for xi in range(0, 128):
                    c = color_f2hex(random.random(), random.random(), random.random())
                    m.set(xi, yi, c)
        elif pattern == "square":
            for yi in range(40, 128-40):
                for xi in range(40, 128-40):
                    if xi < 50 or yi < 50:
                        c = 0xffffff
                    else:            
                        c = color_f2hex(random.random(), random.random(), random.random())
        ...
        offset_canvas.SetPixelsFromMat(m)
