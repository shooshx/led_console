...
from infra_c import RGBMatrix, RGBMatrixOptions
...
def parse_args():
    ...
    d.brightness = 20
    d.gpio_slowdown = 2
    DEFAULTS_4_IN_1_CHAIN = d
    # ----------------------------------

    d = DEFAULTS_4_IN_1_CHAIN

    parser = argparse.ArgumentParser()

    parser.add_argument("-r", "--led-rows", action="store", help="Display rows. 16 for 16x32, 32 for 32x32. Default: 32", default=d.rows, type=int)
    parser.add_argument("--led-cols", action="store", help="Panel columns. Typically 32 or 64. (Default: 32)", default=d.cols, type=int)
    parser.add_argument("-c", "--led-chain", action="store", help="Daisy-chained boards. Default: 1.", default=d.chain_length, type=int)
    parser.add_argument("-P", "--led-parallel", action="store", help="For Plus-models or RPi2: parallel chains. 1..3. Default: 1", default=d.parallel, type=int)
    ...
    parser.add_argument("-b", "--led-brightness", action="store", help="Sets brightness level. Default: 100. Range: 1..100", default=d.brightness, type=int)
    ...
    parser.add_argument("--led-slowdown-gpio", action="store", help="Slow down writing to GPIO. Range: 0..4. Default: 1", default=d.gpio_slowdown, type=int)
    ...
    parser.add_argument("--led-multiplexing", action="store", help="Multiplexing type: 0=direct; 1=strip; 2=checker; 3=spiral; 4=ZStripe; 5=ZnMirrorZStripe; 6=coreman; 7=Kaler2Scan; 8=ZStripeUneven... (Default: 0)", default=d.multiplexing, type=int)
    ...
    parser.add_argument("--led-limit-refresh", action="store", dest="limit_refresh", help="Limit refresh rate.", default=0, type=int)
    ...
    options.limit_refresh_rate_hz = args.limit_refresh
...
class DisplayMatrix(infra.BaseDisplay):
    ...
    def refresh(self):
        self.fps.inc()
        self.offset_canvas.SetPixelsFromMat(self.pixels)
        start = time.time()
        self.offset_canvas = self.matrix.SwapOnVSync(self.offset_canvas)
        print("{:.1f}".format((time.time() - start)*1000))