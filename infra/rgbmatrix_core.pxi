...
cdef class Canvas:
    ...
    # this is a mapping from 4 panels in a row that make a 256x64 buffer into a square 2x2 panels that make a 128x128 buffer
    # arranged like so:
    #              +---+     +---+
    #              | 4 | <-- | 3 |
    #              +---+     +---+
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    def SetPixelsFromMat(self, infra_c.IntMatrix m):
        cdef cppinc.FrameCanvas* my_canvas = <cppinc.FrameCanvas*>self.__getCanvas()
        cdef int yi, xi, dyi
        cdef unsigned int c
        for yi in range(0, 64):
            for xi in range(0, 128):
                ...
                g = (c >> 8) & 0xFF
                b = (c >> 16) & 0xFF
                my_canvas.SetPixel(xi, yi, r, g, b)
        dyi = 0
        for yi in range(64, 128):
            for xi in range(0, 128):
                c = m.c_get(xi, yi)
                r = (c ) & 0xFF
                g = (c >> 8) & 0xFF
                b = (c >> 16) & 0xFF
                my_canvas.SetPixel(127 - xi + 128, 63 - dyi, r, g, b)
            dyi += 1
